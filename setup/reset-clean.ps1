...
# ------------------------------------------------- 5a. antivirus state (read only)
# KidNest does not change antivirus settings, so there is nothing to undo here.
# Report it only, since a quarantined mitmdump explains a lot.
$det = @(Get-MpThreatDetection -EA SilentlyContinue |
         Where-Object { "$($_.Resources)" -match "mitmdump|KidNest" } |
         Select-Object -First 3)
if ($det) { $det | ForEach-Object { Note "defender detection: $($_.Resources)" "seen" } }
else { Note "defender detections for KidNest" "none" }
